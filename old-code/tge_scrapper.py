...
os.makedirs(os.path.dirname('ee_rdn/ee_rdn.csv'), exist_ok=True)
os.makedirs(os.path.dirname('ee_rdb_pln/ee_rdb_pln.csv'), exist_ok=True)
os.makedirs(os.path.dirname('ee_rdb_eur/ee_rdb_eur.csv'), exist_ok=True)
os.makedirs(os.path.dirname('gz_rdn/gz_rdn.csv'), exist_ok=True)
os.makedirs(os.path.dirname('gz_rdb/gz_rdb.csv'), exist_ok=True)
...
for date in date_list:
    if not os.path.exists(f"ee_rdn/EE_RDN_{date}.csv"):
        ...
        if r.status_code == 200:
            ...
            df.reset_index(drop=True,
                           inplace=True)
            df.to_csv(f"ee_rdn/EE_RDN_{date}.csv", index=False)
    ...
    # #GZ - RDN
    if not os.path.exists(f"gz_rdn/GZ_RDN_{date}.csv"):
        ...
        if r.status_code == 200:
            ...
            df.drop(columns=['Chart'],
                    inplace=True)
            df = df.convert_dtypes()
            df.to_csv(f"gz_rdn/GZ_RDN_{date}.csv", index=False)
