...
for key in cities:
    ...
    for i in range(5):
        ...
        if status == 200:
            ...
            df = pd.json_normalize(
                json_response,
                record_path=['weather'],
                meta=['name', ['main', 'temp'], ['main', 'feels_like'], ['main', 'temp_min'], ['main', 'temp_max'],
            ...
            df = df.rename(columns={'main': 'weather_category', 'description': 'weather_description',
                                    ...
                                    'name': 'city'})
            df = df.apply(
                lambda x: datetime.fromtimestamp(int(x)).astimezone(to_zone) if x.name in ['timestamp', 'sunrise',
                                                                                           'sunset'] else x)
            ...
            # saving json to file
            filename = f'weather/weather_{date}.csv'
    ...
    for i in range(5):
        ...
        if status == 200:
            ...
            # saving json to file
            filename = f'forecast/forecast_{date}.csv'
            os.makedirs(os.path.dirname(filename), exist_ok=True)
